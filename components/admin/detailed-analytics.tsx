...
import { 
  ...
  MapPin, ShieldAlert, Monitor, Sparkles
} from "lucide-react"
...
export function DetailedAnalytics({ data, realtimeUsers }: DetailedAnalyticsProps) {
  ...
  const [timelineDays, setTimelineDays] = useState<30 | 7>(30)

  // Filter timeline based on days state
  const timelineData = timelineDays === 7 ? data.timeline.slice(-7) : data.timeline
          ...
          <div className="flex gap-1 bg-gray-100 p-1 rounded-lg">
            <button
              ...
              className={`px-3 py-1 text-xs font-semibold rounded ${
                timelineDays === 7 ? "bg-white text-gray-900 shadow-sm" : "text-gray-500 hover:text-gray-900"
              }`}
            >
              7d
            </button>
            <button
              ...
              className={`px-3 py-1 text-xs font-semibold rounded ${
                timelineDays === 30 ? "bg-white text-gray-900 shadow-sm" : "text-gray-500 hover:text-gray-900"
              }`}
            >
              30d
            </button>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <h3 className="text-4xl font-bold tracking-tight text-gray-900 mt-2">
                      {timelineData.reduce((acc, t) => acc + t.views, 0).toLocaleString()}
                    </h3>
                  </div>
              ...
              <CardContent className="pt-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <h3 className="text-4xl font-bold tracking-tight text-gray-900 mt-2">{data.totalLeads}</h3>
                  </div>
              ...
              <CardContent className="pt-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <h3 className="text-4xl font-bold tracking-tight text-gray-900 mt-2">
                      {data.totalViews > 0 ? ((data.totalLeads / data.totalViews) * 100).toFixed(2) : "0.00"}%
                    </h3>
                  </div>
          ...
          <Card className="shadow-sm">
            <CardHeader className="flex flex-row items-center justify-between border-b pb-4">
              <div>
                ...
                <CardDescription>Daily pageviews and lead form conversions</CardDescription>
              </div>
            </CardHeader>
            <CardContent className="pt-6">
                ...
                <div className="divide-y divide-gray-100">
                  {data.pages.slice(0, 5).map((page, index) => (
                    <div key={page.name} className="flex items-center justify-between p-4 hover:bg-gray-50/50">
                      ...
                      <span className="text-sm font-bold text-gray-900 bg-gray-100 px-2 py-0.5 rounded">{page.value} views</span>
                    </div>
                  ))}
                </div>
              </CardContent>
                ...
                <div className="divide-y divide-gray-100">
                  {data.locations.slice(0, 5).map((loc, index) => (
                    <div key={loc.name} className="flex items-center justify-between p-4 hover:bg-gray-50/50">
                      ...
                      <span className="text-sm font-bold text-gray-900 bg-gray-100 px-2 py-0.5 rounded">{loc.value} visits</span>
                    </div>
                  ))}
                </div>
              </CardContent>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in fade-in-50 duration-200">
          ...
          <Card className="shadow-sm flex flex-col justify-between">
            ...
            <CardContent className="pt-6 pb-2">
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                      ...
                      dataKey="value"
                    >
                      {data.osListBreakdown.map((entry, index) => (
              ...
              <div className="space-y-2 mt-4">
                {data.osListBreakdown.map((os, index) => (
